              ...
              haremList.map((rel, idx) => (
                ...
                >
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <div className="mono text-[8px] text-pink-500/60 font-black uppercase tracking-widest mt-0.5">{toSentenceCase(rel.powerLevel || 'Phàm nhân')}</div>
                    <div className="text-[7px] text-neutral-600 uppercase font-bold mt-1 truncate">{toSentenceCase(rel.faction || 'Vô tông môn')}</div>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="text-pink-500 text-[10px] font-black mono">♥ {renderSafeText(rel.affinity)}%</div>
                  </div>
                </div>
              ))
            ) : (
        ...
        <div className="flex-grow p-10 md:p-20 overflow-y-auto custom-scrollbar relative bg-[radial-gradient(circle_at_center,rgba(236,72_153,0.04),transparent)]">
          {selectedNpc ? (
              ...
              <div className="flex flex-col lg:flex-row gap-12 items-start">
                <div className="flex-shrink-0 space-y-6 text-center group">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <span>📸</span> Chỉnh sửa hồ sơ & Chỉ số
                    </button>

                    <div className="flex justify-center gap-2 mt-4">
                      <span className="px-4 py-1 bg-pink-500/10 border border-pink-500/20 rounded-lg text-[9px] text-pink-400 font-black uppercase tracking-widest">{toSentenceCase(selectedNpc.race || 'Nhân loại')}</span>
                      <span className="px-4 py-1 bg-white/5 border border-white/10 rounded-lg text-[9px] text-neutral-400 font-black uppercase tracking-widest">{toSentenceCase(selectedNpc.mood || 'Bình thản')}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                  <div className="p-8 bg-black/40 border border-white/5 rounded-[2.5rem] relative overflow-hidden group hover:border-pink-500/30 transition-all">
                    ...
                    <span className="block mono text-[9px] font-black text-neutral-600 uppercase mb-4 tracking-[0.2em]">Tâm lý đối với MC</span>
                  ...
                  <div className="p-8 bg-black/40 border border-white/5 rounded-[2.5rem] group hover:border-pink-500/30 transition-all">
                    <span className="block mono text-[9px] font-black text-neutral-600 uppercase mb-6 tracking-[0.2em]">Bí mật khuê phòng</span>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-2xl font-black text-neutral-700 mono uppercase tracking-[0.5em]">Chọn một mỹ nhân</h3>
            </div>
          )}
        </div>
      </div>
