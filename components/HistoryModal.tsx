...
export const HistoryModal: React.FC<Props> = ({ isOpen, onClose, logs, onLoadSave }) => {
  const [slotsInfo, setSlotsInfo] = useState<Record<string, SaveMetadata | null>>({});

  // Nạp thông tin slot lưu trữ tự động khi mở modal
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <div className="fixed inset-0 z-[450] bg-black flex flex-col animate-in fade-in duration-300 mono">
